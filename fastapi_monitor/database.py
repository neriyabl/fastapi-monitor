...
class MonitorDatabase:
    ...
    async def init_db(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_status ON requests(status_code)")
    ...
    async def store_request(
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """, (
                timestamp, method, path, query_params, status_code,
                response_time, request_size, response_size, client_ip,
                user_agent, json.dumps(headers) if headers else None,
    ...
    async def get_recent_requests(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """, (limit, offset)) as cursor:
                ...
                for row in rows:
                    ...
                    # Format timestamp
                    import datetime
                    dt = datetime.datetime.fromtimestamp(row_dict['timestamp'])
            ...
            async with db.execute("SELECT * FROM requests WHERE id = ?", (request_id,)) as cursor:
                ...
                if row:
                    ...
                    if row_dict['response_headers']:
                        row_dict['response_headers'] = json.loads(row_dict['response_headers'])
    ...
    async def get_analytics_data(self, resolution: str = "30s") -> Dict[str, Any]:
        ...
        # Get requests over time with specified resolution
        requests_over_time = await self.get_requests_over_time(resolution)
        
        async with aiosqlite.connect(self.db_path) as db:
            pass  # This method will be replaced by the new one below
    
    async def get_requests_over_time(self, resolution: str = "30s") -> List[Dict[str, Any]]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            if resolution == "30s":
                # Special case for 30-second intervals
                async with db.execute("""
                    SELECT 
                    ...
                    GROUP BY time_slot
                    ORDER BY time_slot
                """, (config["duration"],)) as cursor:
                    return [{"time": row[0], "count": row[1]} for row in await cursor.fetchall()]
            else:
                # For other resolutions, use interval grouping
            ...
            async with db.execute("""
                ...
                GROUP BY range
            """) as cursor:
                response_time_dist = [{"range": row[0], "count": row[1]} for row in await cursor.fetchall()]
            ...
            async with db.execute("""
                ...
                LIMIT 10
            """) as cursor:
                top_endpoints = [{"path": row[0], "count": row[1], "avg_time": round(row[2], 2)} for row in await cursor.fetchall()]
            ...
            """) as cursor:
                status_trends = [{"date": row[0], "status_code": row[1], "count": row[2]} for row in await cursor.fetchall()]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute("SELECT AVG(response_time) as avg_time FROM requests") as cursor:
            ...
            # Error count
            async with db.execute("SELECT COUNT(*) as errors FROM requests WHERE error_info IS NOT NULL") as cursor:
